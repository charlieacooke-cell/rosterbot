...
async function loadOverrides() {
  try {
    ...
    if (data && Object.keys(data).length > 0) {
      // Merge delta on top of base — null entry means "deleted"
      SHIFTS = Object.assign({}, BASE_SHIFTS);
      for (const [iso, info] of Object.entries(data)) {
        if (info === null) delete SHIFTS[iso];
        else SHIFTS[iso] = info;
      }
      refresh();
    }
...
async function saveEdits() {
  ...
  // Compute delta vs BASE_SHIFTS so we only persist overrides, not the whole dataset.
  // This means regenerating the site never loses manual edits, and old stored data
  // from a previous term can't override new base shifts.
  const delta = {};
  for (const [iso, info] of Object.entries(editShifts)) {
    const base = BASE_SHIFTS[iso];
    if (!base || base.code !== info.code) delta[iso] = info;
  }
  for (const iso of Object.keys(BASE_SHIFTS)) {
    if (!(iso in editShifts)) delta[iso] = null; // deleted entry
  }
  ...
  try {
    const res = await fetch('/api/shifts', {
      ...
      body: JSON.stringify({ password: editPassword, shifts: delta }),
    });
